...
public class CustomWeaponMenu extends AbstractContainerMenu {

    public CustomWeaponMenu(int windowId, Inventory playerInventory, FriendlyByteBuf buf) {
        this(windowId, playerInventory, new ItemStackHandler(30));
    }

    public CustomWeaponMenu(int windowId, Inventory playerInventory, IItemHandler ghostInventory) {
        super(ModMenuTypes.CUSTOM_WEAPON_MENU.get(), windowId);
    ...
    public void clicked(int slotId, int dragType, ClickType clickTypeIn, Player player) {
        if ((slotId < this.slots.size() && slotId >= 0 && this.slots.get(slotId).getItem().getItem() instanceof CustomSwordItem)
